...
[2026-08-06][PLAN] Planned the CLP load-more progress and Promotion This Month section from Figma mobile/desktop references, targeting existing paginated-list behavior plus a merchant-editable collection promotion module so the product grid handoff matches the redesign.
[2026-08-06][IMPLEMENT] Added a manual load-more progress bar tied to the existing paginated-list state and created a collection-scoped Promotion This Month section with editable image, offer accordions, and desktop CTA.
[2026-08-06][REVIEW] Compound review found no blocking regressions; noted the standalone promotion section depends on uploading its section file with the collection template and that the real campaign image still needs merchant selection.
